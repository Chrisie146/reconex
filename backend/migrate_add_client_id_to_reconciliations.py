"""
Database migration: Add client_id column to reconciliations table
This enables multi-client support for monthly reconciliations
"""
...
def migrate_database(db_path="statement_analyzer.db"):
    """Add client_id column to reconciliations table"""
    ...
    if "client_id" in columns:
        print("✓ Column 'client_id' already exists in reconciliations table")
        conn.close()
        return
    
    print("Adding 'client_id' column to reconciliations table...")
    ...
    try:
        ...
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_reconciliations_client_id 
            ON reconciliations(client_id)
        """)
        ...
        cursor.execute("SELECT COUNT(*) FROM reconciliations WHERE client_id IS NULL")
