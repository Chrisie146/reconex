    ...
    def encode(self, o):
        if isinstance(o, float):
            if math.isnan(o) or math.isinf(o):
                return str(0.0)  # Convert to string representation of 0.0
...
# Middleware to sanitize NaN/infinity values from responses
class JSONSanitizationMiddleware(BaseHTTPMiddleware):
    """Sanitize NaN and infinity values in responses"""
    async def dispatch(self, request: Request, call_next):
        try:
            response = await call_next(request)
            # Note: By this point the response is already serialized,
            # so we catch the error at the source instead
            return response
...
async def upload_statement(
    ...
    try:
        ...
        if preview:
            # Sanitize amounts to handle NaN and infinity
            import math
            serialized = []
            for t in normalized_transactions:
                amount = t.get("amount")
                # Sanitize NaN and infinity values to 0
            ...
            response_data = {"preview": True, "transactions": serialized, "warnings": parse_warnings or None, "skipped_rows": skipped_rows or None}
            return sanitize_response_data(response_data)
        ...
        for t in normalized_transactions:
            amount = t.get("amount")
            if isinstance(amount, float) and (math.isnan(amount) or math.isinf(amount)):
                logger.warning(f"[UPLOAD] Invalid amount ({amount}) in transaction: {t['description']}, setting to 0.0")
                t["amount"] = 0.0
        ...
        response_data = {"session_id": session_id, "transaction_count": len(normalized_transactions), "categories": sorted(list(categories_found)), "warnings": parse_warnings or None, "skipped_rows": skipped_rows or None}
        return sanitize_response_data(response_data)
...
async def upload_pdf_statement(
    ...
    try:
        ...
        for t in normalized_transactions:
            amount = t.get("amount")
            if isinstance(amount, float) and (math.isnan(amount) or math.isinf(amount)):
                logger.warning(f"[PDF_UPLOAD] Invalid amount ({amount}) in transaction: {t['description']}, setting to 0.0")
                t["amount"] = 0.0
        ...
        response_data = {"session_id": session_id, "transaction_count": len(normalized_transactions), "categories": sorted(list(categories_found)), "bank_source": bank_source, "warnings": parse_warnings or None, "skipped_rows": skipped_rows or None}
        return sanitize_response_data(response_data)
    except HTTPException:
        raise
    except Exception as e:
...
def save_parsed_transactions(
    ...
    try:
        ...
        response_data = {"session_id": session_id, "transaction_count": len(txns), "categories": sorted(list(categories_found))}
        return sanitize_response_data(response_data)
    except HTTPException:
        raise
    except Exception as e:
