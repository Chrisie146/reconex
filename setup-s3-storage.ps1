...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "⚠️  Bucket already exists, skipping..." -ForegroundColor Yellow
    }
} catch {
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️  Could not create access key. You may already have 2 keys (AWS limit)." -ForegroundColor Red
...
Write-Host ""
Write-Host "⚠️  Save these credentials in a password manager!" -ForegroundColor Red
Write-Host "⚠️  Never commit them to git!" -ForegroundColor Red
Write-Host ""
...
$lines = @(
    ...
    "",
    "WARNING: DELETE THIS FILE AFTER ADDING TO RENDER.COM",
    "WARNING: NEVER COMMIT THIS TO GIT"
)
$lines | Out-File -FilePath $credentialsFile -Encoding utf8

Write-Host 'Credentials saved to: ' -NoNewline -ForegroundColor Green
